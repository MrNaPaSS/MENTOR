...
async def test_snapshot_translates_contracts_to_coins():
    ...
    collector._raw["BTCUSDT"] = RawBook()
...
def test_checksum_matches_okx_example():
    """Сумма считается по строкам биржи и приводится к знаковому числу."""
    bids = [("3366.1", "7"), ("3366", "6")]
    asks = [("3366.8", "9"), ("3368", "8")]
    # Считаем тем же способом, что и биржа: bid, ask, bid, ask.
    expected_text = "3366.1:7:3366.8:9:3366:6:3368:8"
    import zlib

    raw = zlib.crc32(expected_text.encode())
    expected = raw - (1 << 32) if raw >= (1 << 31) else raw
    assert checksum(bids, asks) == expected


async def test_bad_checksum_drops_the_book():
    """Сумма не сошлась - книгу собираем заново, а не показываем как есть."""
    collector = make_collector()
    collector._pinned["BTCUSDT"] = 1
    collector._raw["BTCUSDT"] = RawBook()
    collector.state.ensure("BTCUSDT")
