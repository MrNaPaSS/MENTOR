...
def caption_html(text: str, links: list[dict], attach: dict | None) -> str:
    ...
    У ждущей заявки подпись есть по существу: строка с монетой, стороной и
    состоянием. Это не подпись к картинке, а сама заявка словами - для тех,
    кто читает уведомление, не открывая фотографию.

    У позиции, которая уже в рынке, такой строки нет. На её карточке и так
    стоит всё: монета, сторона, плечо, вход, стоп и результат, - и строка под
    фотографией повторяла карточку слово в слово. Остаются картинка и кнопка
    под ней; слова человека, если он их написал, тоже остаются - это его
    подпись, а не наша выдумка.
    """
    body = text_html(text, links)
    kind = str((attach or {}).get("kind", ""))
    if kind == "trade" and isinstance((attach or {}).get("trade"), dict):
        trade = attach["trade"]
        if str(trade.get("state", "")) == "open":
            return body
        card = trade_html(trade)
