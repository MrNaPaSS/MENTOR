...
* **снимок и инкременты в одном канале.** `@incrDepth` присылает сперва полную
  ...
  (`action: "update"`). Непрерывность проверяется по номеру: следующий обязан
  быть на единицу больше предыдущего. Разрыв - книга разошлась с биржей, и её
  надо собирать заново переподпиской, а не «доправлять»;
...
class BingxCollector:
    ...
    def _on_book(self, state, symbol: str, row: dict) -> None:
        ...
        if update_id >= 0 and update_id <= previous:
            ...
            # знает. Это не разрыв: пересобирать книгу из-за дубля значит
            # оставить ученика без стакана на ровном месте.
            return
        if update_id >= 0 and previous >= 0 and update_id != previous + 1:
            ...
            logger.info(
                "Книга BingX %s разошлась (%s после %s) - переподписка",
                symbol,
                update_id,
                previous,
            )
