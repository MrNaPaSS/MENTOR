...
# DEPTH_SLOW ниже.
#
# Потолок - сто монет: биржа держит на соединении двести потоков, а на каждую
# монету их два (стакан и лента). Число задаётся в окружении, SCALPING_TOP_N.
DEFAULT_TOP_N = 50
...
# Какую ленту слушаем.
#
# Обычная (`trade`) везёт каждую сделку своим сообщением: полсотни монет - это
# около двух тысяч сообщений в секунду. Столько в одном соединении не живёт, и
# лечится это не выбором ленты, а раскладкой по нескольким соединениям
# (`TAPE_SOCKETS` в `backend/scalping/binance.py`): после неё ноль обрывов за
# десять минут замера против одного каждые двадцать секунд.
#
# Сжатая (`aggTrade`) была бы дешевле - все сделки одной заявки по одной цене
# одним сообщением, - и код к ней готов: число сделок считается по номерам
# первой и последней (`_trades_in`). Но её тут нет: соединение поднимается, а
# сообщений не приходит ни одного - проверено пробником дважды, с рабочего
# стола и со стороны, одиночным адресом и общим потоком. Менять имя здесь
# бессмысленно, пока `check_binance_stream.py 20 300 aggTrade` показывает нули.
TAPE_STREAM = "trade"
# Как часто спрашиваем суточную сводку.
#
# Это самый дорогой из постоянных запросов: биржа отдаёт четыре числа по всем
# монетам разом и ставит за это сорок единиц веса - как за четыре снимка
# стакана. Каждые десять секунд выходило 240 веса в минуту из 1400, которые
# отведены фоновым запросам, и на живом столе 17 сентября бюджет упирался
# именно в него: снимки стаканов откладывались, а стакан у трейдера оставался
# пустым.
#
# Тридцати секунд хватает. Сводка нужна списку монет - оборот за сутки,
# процент за сутки, число сделок, - и ни одно из этих чисел за десять секунд
# заметно не меняется. Цены в стакане и на графике идут потоком и от этого
# срока не зависят вовсе.
TICKER_INTERVAL = 30.0      # обновление суточной сводки, секунды
ROTATE_INTERVAL = 300.0     # пересмотр состава топа, секунды
PRUNE_INTERVAL = 30.0       # чистка книг от далёких уровней, секунды

# Шаг цикла сбора. Сам по себе запросов не стоит: по нему только сверяются
# сроки дел выше. Держится коротким, чтобы чистка книг шла вовремя, даже когда
# сводку спрашивают раз в полминуты.
LOOP_STEP = 5.0
...
# Глубина снимка фоновой монеты. Сто уровней весят впятеро меньше пятисот, а
# метрикам скринера дальние уровни не нужны: книга всё равно подрезается полосой
# `KEEP_BAND_BP`. На пересборке после обрыва эта разница решает, уложимся мы в
# бюджет биржи или оставим скринер пустым.
SNAPSHOT_LIMIT = 100        # уровней в снимке инструмента из списка
...
class ScalpingCollector:
    ...
    async def _loop(self) -> None:
        """Сводка, состав топа и чистка книг - каждое по своему сроку.

        Шаг цикла короткий, а сроки у дел разные: сводка спрашивается редко
        (она дорогая), состав пересматривается ещё реже - инструменты на
        границе топа менялись бы местами каждые десять секунд, и подписки
        дёргались бы впустую, - а книги чистятся часто, потому что растут сами.

        Раньше цикл спал ровно столько, сколько живёт сводка, и растянуть её
        срок значило растянуть заодно и чистку книг, которая запросов не стоит
        вовсе.
        """
        last_rotate = 0.0
        last_prune = 0.0
        ...
        while True:
            try:
                now = time.monotonic()
                if not self._tracked or now - last_tickers >= self.ticker_interval:
                    last_tickers = now
            ...
            await asyncio.sleep(min(LOOP_STEP, self.ticker_interval))
