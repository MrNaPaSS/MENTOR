...
class TgCodeIn(BaseModel):
    ...
    узнал. Ученик опознаётся по `tg_id` - он же и связывает человека с
    кабинетом.

    `weex_uid` необязателен. Он был обязательным, пока биржа была одна: без
    счёта на WEEX в кабинете было нечего делать. С мультибиржей ученик приходит
    с OKX или BingX, и требовать номер счёта WEEX - значит не пустить его
    вовсе. Прислали - свяжем и его, это по-прежнему полезно.
    """

    tg_id: int
    weex_uid: str = Field(default="", max_length=64)
...
class ProfileOut(BaseModel):
    ...
    journal_delete_allowed: bool = False
    # С какого дня человек в академии. Профиль отвечает на «кто я», и стаж -
    # это единственное, чего нет больше нигде в кабинете.
    created_at: Optional[str] = None
