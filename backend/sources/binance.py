...
def use_client(client: BinanceRest) -> None:
    """Взять готовый клиент - тот же, которым живёт скальпинг.

    Бюджет запросов биржа считает по адресу, а не по клиенту. Два клиента в
    одном процессе ведут два своих счёта, каждый считает себя в пределах, и
    вместе они выбирают чужой: снимки стаканов у скальпинга перестают
    приходить, а причина не видна ни одному из них.
    """
    global _rest
    _rest = client
...
def rest() -> BinanceRest:
    """Клиент биржи. Один на процесс, общий со скальпингом."""
    global _rest
    if _rest is None:
