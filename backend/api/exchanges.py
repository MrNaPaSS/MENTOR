...
async def listing(
    ...
    out = []
    # Только биржи, на которых терминал уже торгует.
    #
    # Неподключённая биржа на витрине кабинета - это строка, с которой ничего
    # нельзя сделать: ни счёт открыть, ни ключи подключить. Ученик читает её
    # как обещание срока, которого мы не давали. Их число названо на главной
    # словами, там это уместно, а здесь - место для действий.
    for one in venues.tradable():
