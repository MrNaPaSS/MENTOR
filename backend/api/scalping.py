...
# Насколько вперёд терпим просьбу о свече, которая ещё не началась.
#
# Часы браузера и сервера расходятся, и терминал успевает спросить следующую
# свечу раньше нас. Отказ на это трейдер видит как ошибку, хотя случилось
# только то, что сделок в ней пока нет.
#
# Минута, а не пара секунд: десяти секунд не хватило - живой стол продолжал
# получать отказ на каждую новую минуту. Дальше минуты это уже не расхождение
# часов, а ошибка запроса, и о ней надо говорить прямо.
FOOTPRINT_EARLY = 60
...
async def footprint(
    ...
    now = int(time.time())
    if start > now + FOOTPRINT_EARLY:
        raise HTTPException(400, "Эта свеча ещё не началась")
    if start > now:
        # Свеча вот-вот начнётся: часы браузера ушли вперёд на пару секунд, и
        # терминал спросил следующую минуту раньше нас. Это не ошибка запроса -
        # сделок в ней просто ещё нет. Раньше отсюда летел отказ, терминал
        # повторял его каждую секунду до конца расхождения, и журнал сервера
        # был в красных строках на ровном месте.
        return _foot_payload(
            sym,
            interval,
            build_footprint({}, time=start, seconds=seconds, tick=0.0, partial=True),
            source="tape",
            exchange=venue,
        )
