...
#
# Часы браузера и сервера расходятся на секунду-другую, и терминал успевает
# спросить следующую минуту раньше нас. Отказ на это трейдер видит как ошибку,
# хотя случилось только то, что в свече пока нет сделок.
FOOTPRINT_EARLY = 10
