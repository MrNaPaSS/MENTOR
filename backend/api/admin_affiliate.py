...
# Балансы - по запросу на каждого реферала, и это самая длинная часть загрузки
# страницы: на сотне рефералов сотня запросов. Меняются они медленно, а
# наставник смотрит их не посекундно - держим полчаса вместо десяти минут.
_TTL_BALANCE = 1800
