...
async def _move_open(
    ...
    if body.stop is not None:
        ...
        # Проверку «стоп по ту сторону входа» здесь не делаем.
        #
        # Она верна для ждущей заявки, но у открытой позиции стоп в безубытке
        # стоит как раз за ценой входа - выше у лонга, ниже у шорта. Запрет по
        # входу не давал перенести стоп в безубыток вовсе. Настоящее
        # ограничение здесь другое - рынок, и его накладывает сама постановка:
        # она подводит стоп к цене, если он оказался не по ту сторону.
        ...
        #
        # Не сняла - снимаем сами, а не просим об этом трейдера.
        #
        # Раньше здесь стоял отказ со словами «снимите лишний в приложении
        # биржи». Это перекладывание своей работы на человека: у нас на руках
        # весь список заявок и все их идентификаторы, а у него - позиция с двумя
        # стопами, чужое приложение и вопрос, какой из них лишний. Снимаем по
        # самой заявке: идентификатор условной биржа кладёт в разные поля, и
        ...
        if stale and await plan_alive(client, live.symbol, stale):
            ...
            for order in left:
                if await cancel_plan(client, live.symbol, order):
                    logger.info("Прежний стоп %s снят при переносе", live.symbol)
                else:
                    done = False

            # Список заявок биржи отстаёт от снятия на мгновение, и спрашивать
            # его сразу после успешного ответа значит поймать уже снятую заявку
            # и напугать трейдера зря. Перепроверяем только то, что снять не
            # удалось.
            if not done and await plan_alive(client, live.symbol, stale):
                logger.warning(
                    "Прежний стоп %s снять не удалось: на позиции их два",
                    live.symbol,
                )
                raise HTTPException(
                    409,
                    "Новый стоп поставлен, но прежний снять не вышло - "
                    "биржа не отвечает на снятие. Проверьте заявки в её приложении.",
                )
