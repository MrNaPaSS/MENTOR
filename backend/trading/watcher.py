...
# Не чаще этого спрашиваем у биржи исполнения ради журнала - на одну сделку.
#
# Отчёт об исполнениях нужен журналу дважды: когда сделка впервые попадает в
# него открытой и когда берётся очередная цель. И то, и другое - редкость, но
# проход по счёту может оборваться до записи в базу, и тогда следующий проход
# пошёл бы на биржу снова. Бюджет запросов биржа считает один на все стопы и
...
class PositionWatcher:
    ...
    def __init__(self, session_factory, http_session_factory, interval: float = POLL_INTERVAL):
        ...
        # Когда по сделке последний раз спрашивали исполнения ради журнала.
        self._journal_at: dict[int, float] = {}
    ...
    async def _apply(
        ...
        # Сделка открыта, а записи в журнале нет: позиция набралась до того,
        # как эта возможность появилась, или сопровождение подняли заново
        # посреди сделки. Заводим запись на ближайшем проходе - иначе идущая
        # сделка попала бы в журнал только при закрытии, как раньше.
        #
        # Зафиксированное спрашиваем у биржи только при заведении и только
        # если цели уже брались: это запрос, и делать его каждым проходом
        # ради числа, которое меняется на взятой цели, незачем.
        if session is not None and trade.status == "open" and not decision.opened:
            if self._journal_row(session, trade) is None:
                if int(trade.takes_hit or 0) > 0:
    ...
    async def _live_journal_fills(
        ...
        Поверх этого - выдержка. Проход по счёту может оборваться до записи в
        базу (биржа не ответила, процесс перезапустили), и тогда следующий
        проход снова увидит сделку без записи и снова пойдёт на биржу. Журнал
        такой цены не стоит: подождёт до следующего круга.

        Не ответила биржа - оставляем прежние числа: показать ноль там, где
        деньги уже зафиксированы, хуже, чем показать вчерашнее.
        """
        now = asyncio.get_running_loop().time()
        last = self._journal_at.get(trade.id)
        if last is not None and now - last < JOURNAL_GAP:
            self._live_journal(session, trade)
            return
        self._journal_at[trade.id] = now
        try:
