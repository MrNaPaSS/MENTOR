...
def trade_roi(pnl: float, margin: float) -> float:
    """Доход сделки в процентах - от залога, а не от оборота.

    Так его считают биржи, и трейдер сверяет карточку с их приложением: плечо в
    двадцать пять раз превращает движение цены на восемь процентов в двести
    процентов на залог, и меньшая цифра выглядела бы обманом в обратную
    сторону.
