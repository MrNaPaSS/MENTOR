...
# Сколько живёт ответ.
#
# Две секунды, а не одна: терминал спрашивает раз в три секунды, а открыт он
# бывает на двух устройствах сразу - при секундной памяти их запросы почти
# всегда расходились мимо неё, и один и тот же счёт спрашивался у биржи вдвое
# чаще. Больше двух нельзя: после взятой цели состояние меняется за доли
# секунды. Всё, что меняет заявки и позиции, память сбрасывает само (WRITES).
TTL = 2.0
