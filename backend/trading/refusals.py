...
# Пара не торгуется через API (WEEX). В приложении биржи она есть, и цена у неё
# идёт - а заявку по ключам не поставить: биржа держит отдельный, куда более
# короткий список пар для API. На сентябрь 2026 это 290 пар из 995 в общем
# справочнике инструментов.
_NO_API_PAIR = re.compile(
    r"not\s+supported\s+via\s+the\s+api|trading\s+pair\s+is\s+not\s+supported",
    re.IGNORECASE,
)
...
def explain(message: str) -> str:
    ...
    if _NO_API_PAIR.search(text):
        return (
            "Эту монету биржа не даёт торговать через ключи. В приложении она "
            "есть, и цена по ней идёт, но заявку по API биржа не примет - "
            "торговать её можно только руками у себя в приложении. Повтор не "
            "поможет, выберите другую монету."
        )
