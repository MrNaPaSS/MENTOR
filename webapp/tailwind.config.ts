...
const config: Config = {
  ...
  theme: {
    extend: {
       ...
       * Слой модальных окон.
       *
       * Шапка кабинета и лента рынка стоят на `z-50`, и окно с таким же
       * номером затемняло страницу, но не их: верх экрана оставался светлым и
       * кликабельным, будто окно открылось где-то сбоку. Один слой на все окна
       * вместо россыпи «z-50, z-[60], z-[70], z-[75]» - чтобы следующее окно
       * не пришлось снова подбирать на глаз.
       */
      zIndex: {
        modal: "80",
      },
      ...
      keyframes: {
        ...
        // Бегущая строка. Сдвиг в пикселях из переменной, а не в процентах:
        // половина ленты меряется по тексту и шириной выходит дробной, а от
        // дробной -50% лента на каждом кадре стоит между пикселями. Крупные
