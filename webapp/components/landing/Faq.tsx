...
const FAQS: FaqItem[] = [
  {
    ...
    tagColor: "#06B6D4",
    q: "Что такое NMNH?",
    a: "NMNH (No Money No Honey) - торговая академия с собственным терминалом, живым комьюнити и аналитикой. Это не канал с сигналами: терминал подключается к твоему счёту на бирже, считает риск и сам ведёт открытую позицию.",
  },
  {
    tag: "Терминал",
    tagColor: "#06B6D4",
    q: "Чем это отличается от обычных сигналов?",
    a: "Сигнал в канале - это картинка, всё остальное ты делаешь руками. У нас сигнал открывается прямо в терминале: объём и маржа уже посчитаны под твой депозит, стоп и цели выставляются на бирже вместе со входом, а дальше сервер сам переносит стоп в безубыток по мере взятия целей.",
  },
  {
    tag: "Безопасность",
    tagColor: "#00D4A0",
    q: "Безопасно ли давать API-ключи?",
    a: "Ключ создаётся только на торговлю, без права вывода средств - снять деньги с твоего счёта по нему нельзя. На сервере ключи хранятся зашифрованными и в браузер не возвращаются: видно только последние четыре символа. Отвязать их можно в один клик - в терминале или на стороне биржи.",
  },
  {
    tag: "Биржа",
    tagColor: "#A855F7",
    q: "Работает только с WEEX?",
    a: "Да, терминал торгует через WEEX: доступ в академию открывает регистрация по партнёрской ссылке, и на ней же построены расчёт лимитов, комиссий и сопровождение сделок.",
  },
  {
    tag: "Стоимость",
    tagColor: "#00D4A0",
