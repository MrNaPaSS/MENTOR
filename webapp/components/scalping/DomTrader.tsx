...
/**
 * Через сколько покоя стакан сам возвращается к свежей колонке.
 *
 * Отмотанная история кластеров держалась, пока её не вернут руками, и это
 * подводило: трейдер разглядел прошлое, отвёл глаза - и работает по стакану,
 * стоящему в позавчера. Двадцать секунд: хватает разглядеть отмотанное и мало,
 * чтобы пропустить движение.
 *
 * Отсчёт идёт от последнего касания стакана, а не от появления новой колонки:
 * пока листают, возврата не будет.
 */
const BACK_TO_LIVE_MS = 20_000;
...
export default function DomTrader({
  ...
  // Когда стакан трогали в последний раз: по этому и считается покой.
  ...
  //
  // Ждём не вечно. Мышь у скальпера и так живёт над стаканом, и одно её
  // присутствие не значит, что он смотрит: через двадцать секунд покоя спред
  // возвращается в середину, даже если курсор на месте.
  useEffect(() => {
    const el = scrollRef.current;
    if (!el || frame.rows.length === 0) return;
    const idle = Date.now() - touchedAt.current;
    if (idle < BACK_TO_LIVE_MS && (hovering.current || Date.now() < holdUntil.current)) return;
  ...
  // Возврат к свежей колонке.
  //
  // Вертикаль возвращается сама - спред держится в середине, - а горизонталь
  // оставалась там, куда её увели: отмотал историю кластеров, отвлёкся, и
  // стакан на экране показывает прошлое. Теперь через двадцать секунд покоя он
  // вернётся к цене сам.
  //
  // Пока мышь над стаканом, ждём: трейдер может разглядывать отмотанное. Но
  // ждём не вечно - мышь у него и так живёт над стаканом, и через полминуты
  // покоя возвращаемся в любом случае.
  useEffect(() => {
    const el = scrollRef.current;
    if (!el) return;

    const timer = setInterval(() => {
      if (stickRight.current) return;
      const idle = Date.now() - touchedAt.current;
      if (idle < BACK_TO_LIVE_MS) return;
      if (hovering.current && idle < BACK_TO_LIVE_MS * 1.5) return;

      el.scrollLeft = el.scrollWidth;
      stickRight.current = true;
    }, 1000);
