...
function tradeBoxes(
  ...
  // Бокс показывает, чем сделка рисковала, и не съёживается вслед за стопом.
  //
  // Стоп после первой цели подтягивают ближе ко входу, и бокс, нарисованный по
  // текущему стопу, схлопывался в полоску: с графика пропадало ровно то, ради
  // чего он и нужен - на что трейдер шёл, когда входил. Поэтому границы всегда
  // от входа до задуманного стопа.
  //
  ...
  // Линия текущего стопа внутри бокса: он уже не там, где задумывался, и без
  // неё непонятно, докуда риск сокращён. Подпись у неё не нужна - её несёт
  // ценовая шкала.
  const segments: Shapes["segments"] =
    Math.abs(trade.stop - trade.initialStop) > 1e-12
      ? [
          {
            fromTime: anchor as UTCTimestamp,
            toTime: span,
            price: trade.stop,
            color: safe ? palette.mtf : palette.askLine,
            dashed: true,
            labelAt: "end",
          },
        ]
