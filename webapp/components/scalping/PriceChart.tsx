...
/**
 * Какая доля окна остаётся пустой справа при заходе на монету.
 *
 * Свеча, прижатая к правому краю, - это график, в котором некуда идти: там
 * рисуется бокс сделки, туда тянут стоп и цели, туда же уходит цена в
 * ближайшие минуты, и всё это упиралось в шкалу. Треть окна впереди - то
 * место, где сделка видна целиком, вместе с тем, чем она кончится.
 *
 * Прокрутке это не мешает: доля работает при заходе на монету и при смене
 * таймфрейма, а дальше график принадлежит руке.
 */
const AHEAD_SHARE = 0.38;
 ...
 *
 * Пять секунд, а не три: одна страница сделок стоит бирже двадцать единиц
 * веса, а на десяти- и пятнадцатиминутных свечах профиль добирается именно ею.
 * При трёх секундах два открытых терминала выбирали минутный бюджет почти до
 * дна, и снимки стаканов скринера не проходили вовсе.
 */
const FOOTPRINT_REFRESH_MS = 5000;
...
/**
 * Где плюсик стоит относительно текущей цены, точки экрана.
 *
 * Ровно на ней: кнопка ростом в двадцать точек, и половина высоты ставит её
 * середину на линию цены. Так они читаются одним: цена и действие с этой цены.
 *
 * Поднимать его выше приходилось, пока он стоял над ценовой шкалой и накрывал
 * собой её подписи. Теперь он левее шкалы, и прятаться ему не от чего.
 */
...
function PriceChart({
  ...
  useEffect(() => {
    ...
    function reframe(total: number) {
      ...
      const ahead = Math.round(VISIBLE_BARS * AHEAD_SHARE);
      chart.timeScale().setVisibleLogicalRange({
        from: Math.max(0, total - (VISIBLE_BARS - ahead)),
        to: total + ahead,
      });
    }
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // Цену берём у последнего нарисованного бара, а не у ленты сделок.
      //
      // Плашку текущей цены библиотека ставит по закрытию последнего бара в
      // ряду, а лента приходит чаще, чем обновляется свеча. У дорогих монет
      // разницу не видно: у биткойна весь экран - это сотни долларов, и тик в
      // десять центов не стоит и доли точки. А у монеты за сто долларов на
      // экране умещается полдоллара, и тот же тик - это уже десяток точек:
      // таймер и плюсик отрывались от линии цены и ехали сами по себе.
      //
      // Лента остаётся запасным вариантом: пока ряд пуст, рисовать не от чего.
      const price = dataRef.current.at(-1)?.close || livePriceRef.current;
    ...
    <div className="relative h-full w-full overflow-hidden">
      ...
      {trades
        ...
        .map((row) => {
          ...
          // ней трейдер сверяется глазами. Забранное по целям видно в журнале,
          // отдельной строкой сделки: смешать их значит показать 219 там, где
          // на счёт пришло 148, а держать на графике два числа рядом с ценой -
          // лишний шум. В подсказке они по-прежнему оба.
