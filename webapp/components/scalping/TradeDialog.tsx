...
export default function TradeDialog({
  ...
  /**
   * Открываемся в левой части графика, по высоте посередине.
   *
   * Посреди экрана окно вставало ровно на цену, от которой его открыли:
   * трейдер правит сумму и стоп, глядя на график, а график в этот момент под
   * окном, и первым движением его каждый раз отодвигали. В левой части свежих
   * свечей нет - там начало видимого куска, - и разметка сделки у правого края
   * остаётся на виду.
   *
   * Считаем от самого графика, а не от края экрана: панели таскают мышью, и
  ...
  const attach = useCallback((node: HTMLDivElement | null) => {
    ...
    const indent = chart.left + chart.width * 0.06;
    const left = Math.max(gap, Math.min(indent, chart.right - self.width - gap));
    const middle = chart.top + chart.height / 2 - self.height / 2;
    const top = Math.max(gap, Math.min(middle, window.innerHeight - self.height - gap));
