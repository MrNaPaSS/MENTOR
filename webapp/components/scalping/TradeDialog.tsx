...
export default function TradeDialog({
  ...
  /**
   * Открываемся у левого края графика, а не посреди экрана.
   *
   * По центру окно вставало ровно на цену, от которой его открыли: трейдер
   * правит сумму и стоп, глядя на график, а график в этот момент под окном, и
   * первым движением его каждый раз отодвигали. У левого края цена бывает
   * редко - там начало видимого куска, - и разметка сделки остаётся на виду.
   *
   * Считаем от самого графика, а не от края экрана: панели таскают мышью, и
   * слева от графика стоят то стакан со скринером, то ничего. Прижатое к краю
   * экрана окно висело в стороне от того, ради чего открылось.
   */
  useLayoutEffect(() => {
