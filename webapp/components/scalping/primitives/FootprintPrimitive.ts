...
/** Отступ картинки от края холста, точки. */
const MARGIN = 4;

/**
 * Место под собственную подпись картинки, точки.
 *
 * Подпись - «шорт столько, лонг столько» и крестик - рисуется страницей над
 * верхней строкой лестницы. Без этого поля картинка, вставшая в самый верх,
 * оказывалась под своей же подписью.
 */
const CAPTION = 20;
 ...
 *
 * Половину: картинка стоит в углу поверх графика, и заняв весь холст, она
 * закрыла бы собой то, ради чего график открыт. Не поместившиеся цены
 * собираются укрупнением шага - строк меньше, но каждая читается.
 */
const RIDE = 0.5;
...
class FootprintPaneView implements IPrimitivePaneView {
  ...
  update() {
    ...
    // Где встанет картинка: посередине пустого поля справа от свечей.
    //
    // Раньше она вставала у своей свечи и висела на цене - и ехала за рынком
    // на каждом кадре: цена на скальпе ходит восемь раз в секунду, и столбец
    // цифр дрожал всё время, пока в него смотрят. Потом она встала в правый
    // верхний угол: дрожь ушла, но угол - это край, и картинка прижималась к
    // ценовой шкале, оставляя между собой и свечами полосу пустоты.
    //
    // Теперь она стоит в середине этой полосы: между последней свечой и краем
    // холста. Поле справа от цены мы оставляем нарочно - там рисуется бокс
    // сделки и туда идёт цена, - и картинка делит его пополам, ничего не
    // закрывая: история цены остаётся слева нетронутой. Связь с рынком - в
    // помеченной строке внутри самой картинки, а не в её положении.
    const scale = chart.timeScale();
    const spacing = scale.options().barSpacing;
    // Последняя свеча ряда: правее неё и начинается пустое поле. Спрашиваем у
    // самой серии - примитив знает только ту свечу, которую разбирают, а она
    // бывает и из середины истории.
    const bars = series.data();
    const lastAt = bars.length > 0 ? (bars[bars.length - 1].time as UTCTimestamp) : null;
    const lastX = lastAt === null ? null : scale.timeToCoordinate(lastAt);
    const from = (lastX ?? 0) + spacing;
    const field = pane.width - MARGIN - from;
    // Не поместилась в поле - остаётся в углу: лучше у края, чем поверх свечей.
    let x = field >= WIDTH ? from + (field - WIDTH) / 2 : pane.width - WIDTH - MARGIN;
    // Сдвиг рукой поверх этого выбора: трейдер потянул картинку за колонку
    // цены, и держать её на месте после этого - значит отменить его решение.
    x = Math.max(MARGIN, Math.min(x + this.source.shift.dx, pane.width - WIDTH - MARGIN));

    // Сверху - место под собственную подпись картинки: «шорт столько, лонг
    // столько». Она рисуется страницей над верхней строкой, и без этого поля
    // уезжала бы под панель инструментов, а картинка - под саму подпись.
    const top = over + CAPTION;
    const y = Math.max(
      top,
      Math.min(top + this.source.shift.dy, pane.height - height - under),
    );
