...
class FootprintPaneView implements IPrimitivePaneView {
  ...
  update() {
    ...
    // Где встанет картинка: в правом верхнем углу холста.
    //
    ...
    // цифр дрожал всё время, пока в него смотрят. Потом мы держали её за
    // цену, запомненную при открытии, - дрожь ушла, но место осталось
    // случайным: где стоял рынок в ту минуту, туда картинка и села, хоть
    // посреди свечей.
    //
    // Угол не двигается вовсе и ничего собой не закрывает: справа от цены
    // пустое поле, ради которого мы его и оставили, а история цены остаётся
    // слева нетронутой. Строка, на которой рынок стоит сейчас, помечена
    // внутри самой картинки - связь с рынком там, а не в её положении.
    let x = pane.width - WIDTH - MARGIN;
    // Сдвиг рукой поверх этого выбора: трейдер потянул картинку за колонку
    // цены, и держать её в углу после этого - значит отменить его решение.
