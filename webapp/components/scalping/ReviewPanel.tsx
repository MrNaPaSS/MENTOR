...
// Разбор: сделки недели папками.
//
// Сверху - неделя и её итог, ниже - позиции, сгруппированные по монете. План
// недели отсюда убран: он нужен раз в понедельник, а место занимал постоянно,
// и разбор упирался в него каждый раз, когда хотелось посмотреть картинки.
//
              ...
              return (
                <section key={coin}>
                  ...
                  {/* Ширину ряда задаёт сама панель, а не ширина экрана: журнал
                      живёт в окне, которое тянут мышью, и брейкпоинты экрана
                      про него ничего не знают - в узкой панели на широком
                      мониторе папки раздувались на пол-окна. */}
                  <div
                    ...
                    {(open ? list : list.slice(0, PAGE)).map((trade) => {
                      ...
                      return (
                        <button
                          ...
                          className="overflow-hidden rounded border border-[var(--pane-border)] text-left transition-colors duration-150 ease-out hover:border-[var(--pane-accent-soft)]"
                        >
