...
export default function DetailView({
  ...
  // Таблица разрезов забирает всё, что осталось до низа окна.
  //
  // Она последняя на экране, прокручивается внутри себя и потому тянется, а не
  // держит высоту числом: иначе под ней оставалось пустое поле в треть экрана,
  // а строк в ней было видно три. Считаем по тому, что заняли ряды выше:
  // высота панели это её содержимое плюс шапка в 48 точек, между рядами по 10.
  // Запас в два десятка точек: панели рядов выше бывают на строку выше
  // своего графика - у краёв периода это подписи «лучшие» и «худшие».
  const above = big + 48 + (mid + 30 + 48) + (low + 48) + 30 + 18;
  const deep = height > 0 ? Math.max(120, height - above - 48) : 150;
