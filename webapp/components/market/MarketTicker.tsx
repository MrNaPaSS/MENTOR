...
/**
 * С какой дельты ленты считаем, что в паре ходят киты.
 *
 * Дельта - разница купленного и проданного за минуту, в деньгах. Триста тысяч
 * чистого перевеса за минуту одним инструментом розница не набирает: столько
 * двигают те, кто заходит объёмом.
 *
 * Знак не важен: кит идёт и вверх, и вниз, а значок говорит о его присутствии,
 * а не о направлении - направление уже сказано процентом рядом.
 */
const WHALE_FLOW = 300_000;
...
const Pair = memo(function Pair({
  ...
  // Киты - по дельте ленты: столько чистого перевеса за минуту розница одним
  // инструментом не набирает.
  const whale = Math.abs(row.delta_notional) >= WHALE_FLOW;
  return (
    ...
    <Link
      ...
      title={[
        ...
        whale ? `крупные деньги в ленте: ${money(Math.abs(row.delta_notional))}` : null,
      ...
      style={{
        background: heavy ? "var(--tick-heavy)" : undefined,
        // Обводка внутренней тенью, а не рамкой: рамка добавила бы паре два
        // пикселя ширины, а по ширине ленты считается скорость её движения -
        // пары дёргались бы при каждом приходе и уходе кита.
        boxShadow: whale ? "inset 0 0 0 1px var(--tick-whale)" : undefined,
      }}
    >
      ...
      {/* Кит - эмодзи: в наборе значков кита нет, а рыба на его месте читалась
          бы не тем. */}
      {whale && (
