...
describe("выбор биржи", () => {
  ...
  it("до ответа сервера берёт запомненную или активную", () => {
    // Пустой список значит «ещё не спрашивали», а не «бирж нет». Отвечать на
    // него пустым выбором - значит сходить на сервер дважды: сперва без
    // биржи, потом с запомненной. Журнал от этого открывался вдвое дольше.
    expect(chooseVenue([], null, "weex")).toBe("weex");
    expect(chooseVenue([], "okx", "weex")).toBe("okx");
    // Не знаем ничего - и фильтра нет: сервер сам скажет, чем разрезать.
    expect(chooseVenue([], null, undefined)).toBe("");
  });
...
describe("выбор уступает смене активной биржи", () => {
  ...
  it("прежняя запись без пометки активную не перебивает", () => {
    // Раньше хранилась одна строка, без памяти об активной бирже. Такой выбор
    // держал журнал на старой бирже и после переключения счёта.
    expect(freshChoice({ venue: "mexc", forActive: "" }, "okx")).toBeNull();
  });
