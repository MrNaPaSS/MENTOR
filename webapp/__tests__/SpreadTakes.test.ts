...
describe("цели после переноса", () => {
  ...
  it("остался третий тейк, а в ответе вся лестница - он не уезжает вниз", () => {
    // Две цели взяты, но в записи сделки они ещё не помечены, и биржа отдаёт
    // все три цены. По номеру третьей цели доставалась цена первой: линия
    // съезжала вниз и вставала на место только на следующем круге опроса.
    const next = spreadTakes(trade("long", [80_200, 80_400, 80_600], 2), 0, 80_900, [
      80_200, 80_400, 80_900,
    ]);
    expect(next.targets).toEqual([80_200, 80_400, 80_900]);
  });

  it("у шорта взятые цели из ответа тоже вычитаются", () => {
    const next = spreadTakes(trade("short", [79_800, 79_600, 79_400], 2), 0, 79_100, [
      79_800, 79_600, 79_100,
    ]);
    expect(next.targets).toEqual([79_800, 79_600, 79_100]);
  });
