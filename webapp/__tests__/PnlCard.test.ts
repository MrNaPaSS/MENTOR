...
describe("доход карточки", () => {
  ...
  it("незакрытая сделка ставит прочерк вместо цены выхода", () => {
    // Прочерк, а не пустое место: пустая строка в колонке читается как сбой
    // вёрстки, а не как «биржа ещё не отчиталась».
    const card = cardFromTrade(trade({ exit_price: null }));
    expect(card.rows).toContainEqual(["Цена выхода", "-"]);
  });
