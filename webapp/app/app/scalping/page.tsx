...
// Сколько ждать перед снимком входа, чтобы объёмный профиль успел собраться.
// Вход - не про момент, а про картину перед ним, и эти две секунды ничего не
// стоят; цель и выход ждать нельзя.
const ENTRY_SHOT_WAIT_MS = 2000;
...
export default function ScalpingPage() {
  ...
  async function autoShot(clientId: string, note: string, stage: ShotStage) {
    ...
    //
    // Он не о моменте, а о том, как выглядел рынок перед входом: объёмный
    // профиль справа собирается позже свечей, и снятый сразу вход оставался с
    // пустой колонкой. Цель и выход ждать нельзя - там свеча уходит за
    // секунду, и в журнале останется картинка «уже после».
    if (stage === "entry") {
      await new Promise<void>((done) => setTimeout(done, ENTRY_SHOT_WAIT_MS));
    }
