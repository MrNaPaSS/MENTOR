...
const TIMEFRAMES = ["1m", "5m", "10m", "15m", "1h", "4h", "12h", "1d"];
...
// Сколько ждать разметку входа, прежде чем снимать как есть.
//
// Вход - не про момент, а про картину перед ним, поэтому его снимок можно
// задержать. Ждём не отмеренные секунды, а появление самой разметки: биржа
// подтверждает позицию не мгновенно, и её блок с целями и стопом приезжает на
// график через секунду-другую после события.
const ENTRY_SHOT_WAIT_MS = 6000;

// Как часто спрашивать, приехала ли разметка.
const ENTRY_SHOT_STEP_MS = 150;
...
export default function ScalpingPage() {
  ...
  useEffect(() => {
    ...
    async function check() {
      ...
      for (const trade of missed) {
        ...
        // Сервер уже записал сделку в журнал - ждать нечего.
        //
        // Он объявляет закрытие не по пустому ответу, а по исполнениям с
        // биржи: в списке закрытых лежат настоящие цена выхода и итог. Это
        // сильнее любой нашей выдержки, и держать разметку на графике ещё
        // десять секунд после этого незачем - трейдер видел, как стоп
        // сработал, и ждал полминуты неизвестности.
        const settled = booked.get(trade.id);

        // Сервер ещё считает сделку живой, а биржа позицию уже не показывает.
        // Просим проверить сейчас: его круг идёт раз в пять секунд, и всё это
        // время терминал не знает, закрылась сделка или биржа запнулась.
        if (!settled && alive) nudge();
  ...
  async function autoShot(clientId: string, note: string, stage: ShotStage) {
    // Даём графику дорисовать свежую разметку.
    //
    // Стоп и цели двигают прямо в сделке, и событие приходит раньше, чем линии
    // успевают переехать. Снимок, сделанный в тот же миг, показывал бы старые
    // уровни - то есть врал бы о сделке. Кадр отрисовки стоит миллисекунды и
    // цену не двигает.
    await new Promise<void>((done) =>
      requestAnimationFrame(() => requestAnimationFrame(() => done())),
    );

    // Вход - единственный снимок, который можно подождать.
    //
    // Он не о моменте, а о картине перед входом, и снимать его до того, как на
    // график лёг блок сделки, бессмысленно: в журнале остаётся голый график
    // без входа, стопа и целей. Ждём саму разметку, а не отмеренное время -
    // биржа подтверждает позицию когда подтвердит.
    //
    // Цель и выход ждать нельзя: там свеча уходит за секунду, и снимок
    // показал бы рынок «уже после».
    if (stage === "entry") {
        ...
        {symbol ? (
          <>
            ...
            >
              ...
              {/* Метка для окна расчёта: оно открывается слева от графика, а
                  где сегодня граница графика, знает только раскладка - панели
                  таскают мышью. */}
              <div data-chart-pane className="relative min-h-0 flex-1 p-1">
