...
export default function ScalpingPage() {
  ...
  async function autoShot(clientId: string, note: string, stage: ShotStage) {
    // Снимаем в тот же миг, когда событие случилось.
    //
    // Ожидание пробовали - и оно вредит. Снимок сделки нужен таким, каким
    // рынок был в момент касания цели или стопа: за полсекунды свеча уходит,
    // и вместо причины в журнале остаётся картинка «уже после». Пусть лучше
    // колонка объёмов на снимке окажется неполной, чем неверной будет сама
    // свеча - её не восстановить ничем.
    const taken = shotRef.current?.();
