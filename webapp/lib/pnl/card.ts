...
export async function render(
  ...
  // печать поверх напечатанного листа. И только вместе с верхней печатью: на
  // листе без неё оба оттиска ставит разметка - движением, один за другим.
  if (stamp) await putMark(ctx, canvas.width, canvas.height, variant);
  return canvas;
}

/** Наш оттиск - тот же, что заверяет бланк сигнала. */
export const MARK_SRC = "/cards/signal-stamp.png";

/**
 * Место оттиска-логотипа: планета в правой части нижней панели.
 *
 * Считаем от панели, а не от края листа: у заготовок она и на разной высоте,
 * и разной ширины, а планета нарисована в ней на одном месте - в правой
 * четверти, посередине высоты. Где планеты нет (кит, кот), там на этом месте
 * ночной город, и оттиск ложится на него так же. Надписи панели кончаются
 * левее - до 0,73 её ширины, - и оттиск их не трогает.
 *
 * Доли те же для холста, для окна карточки и для страницы по ссылке: считает
 * их одна эта функция, а сервер получает готовые числа.
 */
export function sealFrame(variant: Variant): Frame {
  const { panel } = variant;
  const w = panel.w * 0.26;
  const h = panel.h * 0.92;
  return {
    x: panel.x + panel.w * 0.86 - w / 2,
    y: panel.y + panel.h * 0.5 - h / 2,
    w,
    h,
  };
}
...
/**
 * Знак в нижнем правом углу карточки - на планете нижней панели.
 *
 * Тот же оттиск, что стоит на бланке сигнала, и поставлен так же: крупно,
 * плотно, косо и со свечением. Карточка уходит в чужие чаты, и по ней должно
 * быть видно, чем она сделана. Наборная печать в рамке слева вверху остаётся -
 * она про другое: там сказано, что сделку подтвердил терминал.
 *
