...
/**
 * Цвета роста и падения для панелей терминала.
 *
 * Палитра выбирается ради свечей, но рост и падение нарисованы не только на
 * графике: тем же зелёным и красным светятся строки стакана, ленты сделок,
 * скринера и карточки сделки. Пока панели держали свои цвета, выбранная
 * палитра обрывалась на краю графика - слева стоял один зелёный, справа
 * другой, и оба назывались «рост».
 *
 * Возвращаются переменные, а не готовые классы: панели уже написаны через
 * `--pane-up` и `--pane-down`, и подменять достаточно их - каждая панель
 * возьмёт своё, включая полупрозрачные заливки.
 *
 * У стандартной палитры своих цветов нет: там рост и падение приходят от
 * листа - на белом это чёрное и белое, и стакан из них не собрать. Поэтому
 * пусто: панели остаются на цветах темы.
 */
export function paneInk(
  palette: ChartPaletteName,
  paper: ChartPaper,
...
function inkVars(bull: string, bear: string): Record<string, string> {
  return {
    "--pane-up": bull,
    "--pane-down": bear,
    "--pane-up-faint": rgba(bull, 0.06),
    "--pane-down-faint": rgba(bear, 0.06),
    "--pane-up-soft": rgba(bull, 0.2),
    "--pane-down-soft": rgba(bear, 0.2),
    "--pane-up-strong": rgba(bull, 0.4),
    "--pane-down-strong": rgba(bear, 0.4),
    "--pane-up-bar": rgba(bull, 0.7),
    "--pane-down-bar": rgba(bear, 0.7),
  };
}
