 ...
 *
 * Живёт здесь, а не в графике: по этому же списку панель инструментов решает,
 * гасить ли кнопку разбора свечи.
 */
const FOOTPRINT_INTERVALS = new Set(["1m", "3m", "5m", "10m", "15m", "30m", "1h"]);

/** Раскрывается ли свеча этого таймфрейма объёмом. */
export function hasFootprint(interval: string): boolean {
  return FOOTPRINT_INTERVALS.has(interval);
}
...
/**
 * Шаг строк под заданное их число.
 *
 * Карточка объёма - это список в интерфейсе, а не разметка по ценовой шкале:
 * высоту строки задаёт вёрстка, и укрупнение считается не от масштаба графика,
 * а от того, сколько строк карточка готова показать. Свеча на триста шагов
 * биржи иначе превратилась бы в бесконечную ленту, по которой ничего не видно.
 *
 * Целым числом биржевых шагов - по той же причине, что и в `pickStep`.
 */
