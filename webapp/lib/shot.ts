...
const THEMES = {
  ...
  // Без неё картинка сливается с фоном переписки, куда её вставляют.
  dark: {
    bg: "#0B0E11",
    head: "#181A20",
    line: "#2B3139",
 ...
 * Знак для подписи.
 *
 * Загружается один раз и остаётся в памяти: снимки делают подряд, и ждать
 * картинку каждый раз незачем. Не загрузился - подпись обойдётся без знака,
...
export function composeShot(
  ...
  // Справа - кто и когда. Чужой график без времени невозможно ни проверить,
  // ни обсудить.
  ...
  // Подпись под графиком слева: знак и адрес. По ним снимок узнают, куда бы
  // его ни переслали.
