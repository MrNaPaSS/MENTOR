...
/**
 * Адрес из ответа сервера в пригодном для отправки виде.
 *
 * Сервер отдаёт ссылку на снимок полной - он единственный, кто знает, каким
 * именем его видно снаружи. Но старые ответы приходили путём от корня, и
 * дописывать к ним адрес по-прежнему нужно. Разбираем оба случая здесь, а не
 * в каждом месте, где ссылку получают: приписанный к полному адресу второй
 * адрес даёт ссылку, которая никуда не ведёт, и замечают это уже в чужом чате.
 */
...
export interface Profile {
   ...
   *
   * Отдельно от `username`: тот приходит из Telegram и переписывается при
   * каждом входе, а карточку показывают другим. Пусто - подпись берётся из ника.
   */
   ...
   *
   * По той же причине приходит вместе с профилем: корзину у записи рисует
   * только тот, кому это разрешено, и знать об этом надо до отрисовки.
   */
...
export interface StudentOut {
  ...
  is_vip: boolean;
  /** Откуда VIP: referral - сам, рефералу академии; manual - наставником. */
  vip_source?: string;
  coins: number;
   ...
   *
   * Вход и присутствие - разное: человек заходит утром и работает весь день,
   * и по времени входа он «заходил восемь часов назад», хотя сидит в
   * терминале прямо сейчас.
   */
