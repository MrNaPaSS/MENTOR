...
export type JournalTrade = {
  ...
  fee: number;
  /**
   * Плата за финансирование: минус - платил, плюс - получал.
   *
   * Уже входит в `pnl`, как её считает и сама биржа, и отдельной строкой не
   * показывается: в карточке стоят результат и комиссия, как и раньше. Поле
   * приходит, чтобы по записи можно было объяснить, из чего сложился итог, -
   * на долгой сделке эта плата и есть вся разница с нашим расчётом.
   */
  funding?: number;
  ...
  /**
   * Итог сделки - то самое число, которое показывает биржа.
   *
   * С платой за финансирование, но до комиссии: так его считает сама биржа в
   * карточке позиции, и журнал обязан сходиться с приложением до копейки.
   * Комиссия стоит рядом отдельной строкой, а не прячется внутрь.
   */
  pnl: number;
  ...
  shots?: TradeShot[];
  /** Разбор словами: что сработало, что нет, почему вошёл. */
  review?: string;
  /** По плану или нарушение. Пусто - трейдер не отмечал. */
  plan_ok?: boolean | null;
  /** Какие правила нарушены: короткие коды. */
  mistakes?: string[];
