...
export type ChatAuthor = {
   ...
   *
   * Отличается от подписи в ленте у наставника: там школа - сигналы идут от
   * NMNH, - а карточка это результат человека, и подписывается она им самим.
   * Старые сообщения приходят без этого поля, поэтому оно необязательное.
   */
  card?: string;
};
...
export type SharedTrade = {
  ...
  pnl?: number | null;
  /**
   * Уже зафиксировано взятыми целями - деньги, которые на счёте.
   *
   * У идущей сделки это главное число карточки: плавающее по остатку через
   * час после отправки уже неправда, а взятое целями не меняется.
   */
  locked?: number | null;
  takesHit?: number;
