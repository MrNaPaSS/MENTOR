...
export type ExchangeAccount = {
  ...
  access?: string;
  /**
   * Биржу можно подключить: академия подтвердила счёт ученика на ней.
   *
   * Пока подтверждения нет, подключать нечего - человек называет UID своего
   * счёта в боте академии, владелец подтверждает, и биржа открывается.
   * Сервер, собранный до этого правила, поля не присылает - тогда считаем, что
   * биржа открыта, и ведём себя как раньше.
   */
  may_connect?: boolean;
};
...
export type SymbolLimits = {
  ...
  max_qty: number;
  /** Потолок всей позиции по монете. Верен только для малого плеча. */
  max_position: number;
  /**
   * Пределы позиции по плечам, в монете: плечо -> предел.
   *
   * Биржа держит предел ступенями, а в справочнике их нет. Сервер запоминает
   * число из отказа «position exceed max size» - после первого же отказа оно
   * известно всем.
   */
  leverage_caps?: Record<string, number>;
  symbol?: string;
};
...
export type ServerTrade = {
   ...
   * Биржа сделки.
   *
   * Ответ идёт по всем биржам сразу: сопровождение ведёт каждую сделку там,
   * где она открыта. На графике чужой биржи такую сделку рисовать нельзя -
   * человек переключился на другой счёт и видел позицию с прежнего вместе с
   * её прибылью.
   */
  exchange?: string;
  /**
   * Позиции на бирже уже нет, сервер дописывает журнал.
   *
   * Отчёт об исполнениях биржа заполняет не мгновенно, и запись ждёт его до
   * сорока секунд. Разметке на графике ждать вместе с журналом нечего: стоп
   * сработал, защита с биржи снята, и бокс сделки на экране - это ложь.
   */
  closing?: boolean;
  qty: number;
  entry: number;
  /** Стоп, который стоит сейчас: после первой цели сервер двигает его сам. */
  stop: number;
  /** Стоп, с которым сделка задумывалась. По нему считался риск. */
  initial_stop: number;
  targets: number[];
  leverage: number;
  margin: number;
...
export type ClosedTrade = {
  ...
  /** Биржа сделки: итог чужой биржи терминал не показывает. */
  exchange?: string;
};
