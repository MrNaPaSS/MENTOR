...
class Student(Base):
    ...
    # VIP: все инструменты терминала без покупки. Выдаётся сам рефералу
    # партнёрки - тому, кто зарегистрировал счёт через академию, - или
    # наставником поимённо (core/referral.py).
...
class ScalpTrade(Base):
    ...
    Пишется с момента открытия и живёт до закрытия: пока `closed_at` пуст,
    сделка в работе. Раньше сюда попадали только закрытые, и трейдер, взявший
    две цели из трёх, не видел в журнале ничего - а зафиксированные деньги у
    него уже были. Итог такой записи (`pnl`) - только то, что **закрыто на
    бирже**: плавающее по остатку живёт в терминале и журналом не считается.

    Всё, что складывает и считает проценты - итоги периода, календарь,
    аналитика, монеты за результат, - берёт только закрытые: недосчитанная
    сделка исказила бы и сумму, и долю прибыльных.
    ...
    outcome: Mapped[str] = mapped_column(String(8))            # open | stop | take | manual
    # Результат. У закрытой - итоговый, у сделки в работе - зафиксированное
    # взятыми целями, за вычетом уже удержанной комиссии.
    pnl: Mapped[float] = mapped_column(Numeric(20, 8), default=0)
    # Сколько объёма уже закрыто. По нему видно, какая часть сделки отработала:
    # «взято две цели» и «закрыто 60% позиции» - разные вещи, когда цели
    # разного размера.
    closed_qty: Mapped[float] = mapped_column(Numeric(24, 10), default=0)
    ...
    # Плата за финансирование, накопленная за жизнь позиции. Знак биржи: минус
    # - платил, плюс - получал.
    #
    # Короткие сделки её не платят вовсе - расчёт идёт раз в несколько часов, -
    # и потому журнал сходился с биржей на них и расходился на долгих. У шорта
    # TAO, провисевшего 34 часа, набежало -25.24 при результате 1287.80: ни в
    # расчёте, ни в комиссии этих денег нет, а со счёта они ушли.
    #
    # Входит в `pnl`: биржа в карточке позиции показывает результат вместе с
    # ней, и журнал обязан показывать то же число. Отдельным полем - чтобы было
    # видно, во сколько обошлось держать позицию так долго.
    funding: Mapped[float] = mapped_column(Numeric(20, 8), default=0)
    opened_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    # Пусто - сделка ещё в работе. Индекс общий со student_id: журнал за
    # период спрашивается им же.
    closed_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), index=True, nullable=True
    )
    note: Mapped[str] = mapped_column(String(255), default="")
    # Разбор сделки словами: что сработало, что нет, почему вошёл.
    #
    # Отдельно от `note`: там служебная отметка о происхождении записи, а это
    # то, ради чего журнал ведут - разговор с собой о собственной сделке.
    review: Mapped[str] = mapped_column(Text, default="")
    # Сделка по плану или нарушение. Пусто - трейдер не отмечал.
    #
    # Считать это сама система не может: она видит цифры, а не намерение.
    # Выдуманный процент дисциплины хуже отсутствующего - по нему будут судить
    # о работе.
    plan_ok: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    # Какие правила нарушены: короткие коды через запятую (`late`, `risk`,
    # `session`, `average`, `fomo`). По ним считаются ошибки недели.
    mistakes: Mapped[str] = mapped_column(String(120), default="")
...
class TradeShot(Base):
    ...
    # Этап сделки, к которому относится снимок: `before`, `entry`, `manage`,
    # `exit`, `review`. Пусто - этап не назван.
    #
    # Ради него всё и затевалось: картинка с этапом перестаёт быть картинкой и
    # становится точкой в истории сделки. Снимок, сделанный терминалом самим,
    # знает свой этап без человека - он снимает по событию.
    stage: Mapped[str] = mapped_column(String(12), default="")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
...
class LiveTrade(Base):
    ...
    # Плата за финансирование по этой позиции, как её назвала биржа.
    #
    # Снимается на каждом обходе, пока позиция жива: после закрытия она
    # исчезает вместе со своими числами, и спросить будет уже негде. В журнал
    # уходит последнее увиденное значение.
    funding: Mapped[float] = mapped_column(Numeric(20, 8), default=0)
