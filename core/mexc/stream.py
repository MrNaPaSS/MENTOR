...
PING_INTERVAL = 15.0
# Сколько поток может молчать и считаться живым: минуту - столько же молчания
# биржа сама считает обрывом.
STALE_AFTER = PING_INTERVAL * 4
...
class MexcPrivateStream:
    ...
    @property
    def ready(self) -> bool:
        """Соединение живо, вход принят, снимок позиций получен и поток отвечает.

        Отвечает - значит присылал что-то недавно: на наш ping биржа отвечает
        раз в PING_INTERVAL. Тихо оборвавшийся сокет долго считает себя
        открытым, и сопровождение видело бы по нему застывшие позиции.
        Молчит дольше STALE_AFTER - позиции спрашиваются у биржи напрямую.
        """
        return (
            bool(self._ws)
