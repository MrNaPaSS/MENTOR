...
class BingxFutures:
    ...
    # Ставит ли биржа приложенную защиту при частичном исполнении лимитки,
    # документация не говорит (ТЗ BingX, §6). Пока не проверено на демо,
    # считаем как у OKX - защита ждёт полного исполнения, - и сопровождение
    # ставит стоп на набранный объём само (`watcher._ensure_stop`). Ошибка в
    # эту сторону даёт лишний стоп, который снимется первым же переносом;
    # ошибка в другую - позицию без стопа.
