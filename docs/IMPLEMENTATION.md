...
├── app/                    # page.tsx (лендинг), login/, app/analysis/
...
- **Логин (`/login`):** двухшаговый вход WEEX UID → код (с auth-flow бэкенда; в dev код
  подсказывается).
- **Кабинет (`/app/analysis`):** главный раздел — разборы сделок; рядом рынок, аналитика, сигналы, профиль.
  Прежний дашборд убран, его адрес переводит на анализы.
...
- **Rate limiting** (`backend/ratelimit.py`): скользящее окно по IP+пути на `/api/auth/*`
  (дефолт 10 попыток / 15 мин, ТЗ §4.3, A-08); preflight `OPTIONS` исключён. Настройки —
  `RATE_LIMIT_MAX`, `RATE_LIMIT_WINDOW`.
- **CORS**: список доменов из `ALLOWED_ORIGINS` (прод — домен фронта вместо `*`).
