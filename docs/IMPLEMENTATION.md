...
**139 Python-тестов (95% покрытие)** + **7 фронтенд-тестов (Vitest)**, все зелёные. WEEX работает на
моках (`WEEX_USE_MOCK=true`) — реальные вызовы подключаются позже без изменения логики.
...
- **Логин (`/login`):** двухшаговый вход WEEX UID → код (с auth-flow бэкенда; в dev код
  подсказывается).
- **Дашборд (`/app/dashboard`):** каркас кабинета с защитой по токену.
