...
Пока туннель закрыт, Cloudflare отвечает за сервер сама: **Error 1033 — Argo
Tunnel error**. Кабинет от этого закрыт экраном «сервер обновляется»
(`webapp/components/app/ServerUpdating.tsx`) — страница-то у человека уже
открыта, и заслонку рисует она сама. Со ссылками на снимки так не выйдет:
страницу снимка отдаёт бэкенд, и пока его нет, до нашего кода запрос не
доходит. А открывают эти ссылки посторонние — те, кому график прислали.

Ответить вместо выключенного сервера может только тот, кто стоит перед ним, —
край Cloudflare. Для этого снимки живут на своём имени `s.nmnh.trade`, и перед
ним стоит воркер `deploy/cloudflare/shots-worker.js`: живые ответы он пропускает
насквозь, а 520–530 подменяет страницей «сервер обновляется» с тем же роликом,
что и на сайте.

Почему отдельное имя, а не `api.nmnh.trade`: снимки открывают люди — это
десятки запросов в день, — а в API ломится сам терминал. Бесплатный тариф
Workers считает запросы посуточно, и посредник перед API однажды положил бы
терминал руками.

Порядок:

1. **DNS.** В Cloudflare → DNS добавить `CNAME s → <TUNNEL-ID>.cfargotunnel.com`,
   прокси включён (оранжевое облако). Либо `cloudflared tunnel route dns nmnh-api s.nmnh.trade`.
2. **Туннель.** В `config.yml` добавить правило `s.nmnh.trade` — оно уже есть в
   `deploy/cloudflared/config.example.yml`. Перезапустить `cloudflared`.
3. **Воркер.** Cloudflare → Workers & Pages → Create Worker, вставить
   содержимое `deploy/cloudflare/shots-worker.js`, задеплоить. Затем
   Worker → Settings → Domains & Routes → Add route: `s.nmnh.trade/*`, зона
   `nmnh.trade`.
4. **Бэкенд.** В `.env`: `SHOTS_BASE_URL=https://s.nmnh.trade`. Перезапустить
   `uvicorn`.

Проверка: остановить `uvicorn` и открыть любую ссылку на снимок — вместо 1033
должна быть чёрная страница с роликом и надписью «Сервер обновляется». Поднять
`uvicorn` обратно — страница вернётся к снимку сама, без нажатий.

Ролик и обложку заслонка берёт с сайта (`/maintenance.mp4`, `/maintenance.jpg`)
— хостинг фронта работает и тогда, когда рабочий стол выключен.
