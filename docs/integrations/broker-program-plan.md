...
> Состояние на 2026-09-12: получены ответы брокерской команды WEEX (§3), главный вопрос
> про условные заявки снят. Код не менялся: сначала сбор данных и заявки биржам (этап 0),
...
|---|---|---|---|---|
| **WEEX** | `newClientOrderId` начинается с `b-WEEX123456-`, ≤ 64 символов (спот и фьючерсы) | Bronze/Silver/Gold: 50/60/70% со своих и трейдеров без пригласившего, 8/11/15% с чужих рефералов | Нужно участие в партнёрке - у NMNH есть. Суб-аффилиаты и их приглашённые брокерской комиссии не дают. На старте нет требований к обороту. API: CheckUserEligibility, GetBrokerCommissionRecords, GetBrokerRebateRatio | ответы брокерской команды получены 12.09.2026, заявка не подана |
...
### Ответы WEEX от 12 сентября 2026

Письмо брокерской команды в ответ на вопросы §8. Главное: **условные заявки засчитываются**,
и **делиться ребейтом с трейдером можно** - оба ответа снимают риски, на которых держался
весь расчёт.

| Вопрос | Ответ WEEX | Что это значит для нас |
|---|---|---|
| Стартовая ставка и уровни | Обычно **50%**, точное значение подтверждает BD-менеджер. Выше 65% - отдельная заявка | Расчёт §2 строился на 50%, менять его не нужно. Уровни и пересмотр - к BD |
| Нынешние рефералы NMNH | Остаются на правилах партнёрки. **Брокерский ребейт и партнёрская комиссия не складываются** | По каждому ученику действует что-то одно. В учёте нужен признак, по какой программе он идёт, иначе кэшбэк посчитается дважды |
| Трейдер с чужим реф-кодом или сам по себе | Действуют правила непрямой комиссии, привязку и право на ребейт подтверждает BD | До ответа BD считаем, что такие обороты приносят меньше или не приносят вовсе; в экономике не закладываемся |
| **Условные заявки** | **Засчитываются** - стопы и цели тоже. Метку брокера класть в `clientAlgoId` | Вопрос №1 снят. Терминал ставит защиту сразу после входа, и половина оборота была под угрозой. Метка идёт в `clientAlgoId` (≤ 32 знаков) наравне с `newClientOrderId` |
| `checkUserEligibility` | Проверяет право пользователя на комиссию. Кто приносил комиссию **до** нашего приглашения, проверку не проходит; в ответе есть причина отказа | Проверяем UID до того, как обещать кэшбэк, и храним причину отказа (`eligibility_reason` в `exchange_accounts`) |
| Ключи брокера | **Отдельные от партнёрских**, выдаются по заявке брокера | В окружении появляется вторая пара ключей WEEX. Партнёрские остаются: по ним считаются обороты и балансы рефералов |
| Делиться ребейтом с трейдером | **Можно**, внутренним переводом по UID. Лимиты - у BD | Наш кэшбэк законен на WEEX (в отличие от Binance, где он запрещён). Способ выплаты - перевод на UID, как и планировалось в `cashback_payouts` |
| Куда и когда приходит комиссия | На счёт, с которого подана заявка брокера. Записи - в партнёрском кабинете и через `GetBrokerCommissionRecords`. Частота выплат - у BD | Сверка начислений идёт по той же ручке, что заложена в §4.4 |

**Что нужно, чтобы подать заявку.** Инструкция на странице брокера, UID нашей партнёрки
(лежит у наставника и в окружении сервера, в публичный документ его не пишем), данные
NMNH.TRADE и цифры доступного торгового оборота.

**Осталось выяснить у BD-менеджера** (не у поддержки): точная ставка и уровни, привязка
трейдеров с чужим реф-кодом, лимиты внутренних переводов, частота выплат.

### Что в документации брокера (проверено 12 сентября 2026)

Ссылки из письма открыты, вот что в них написано.

**Broker ID у нас ещё нет.** Он выдаётся только после заявки: форма на сайте, BD-команда
смотрит её в течение трёх рабочих дней, при одобрении присылает письмо с идентификатором
вида `WEEX` плюс шесть цифр (`WEEX123456`), начальной ставкой и доступом к API. Отдельно
создаётся API-ключ с правами на чтение, спот и фьючерсы. То есть подключать пока нечего:
письмо поддержки - это приглашение подать заявку, а не выдача доступа.

**Метка заявки.** `newClientOrderId` начинается с `b-{brokerId}`, пример из документации -
`b-WEEX123456-20260319001`, длина до 64 знаков, одинаково для спота и фьючерсов. Про
`clientAlgoId` в документации не сказано; то, что условные заявки метятся им и
засчитываются, известно только из письма (§выше), и это стоит подтвердить у BD письменно.

**Ставки в документации не опубликованы.** Есть ручка `GetBrokerRebateRatio` (только для
брокера) - реальную ставку узнаём после доступа. Публично WEEX пишет о «50-70% доли
комиссии» и о более низком пороге оборота для верхнего уровня, чем у конкурентов; названий
уровней и разделения ставок по происхождению трейдера в открытых материалах нет.
Разбивка 50/60/70 и 8/11/15 в таблице §3 взята из публикации BeInCrypto, а не из
документации и не из письма - до подтверждения BD считать её оценкой.

**Отчёт о комиссии.** `GET /api/v3/apiReferral/rebate/recentRecord`, вес запроса 5.
Параметры: `symbol` и `coin` обязательны, `productType` (`SPOT` или `FUTURES`), `page`,
`pageSize` (до 100), `startTime` и `endTime` в виде `yyyy-MM-dd`, окно не больше 90 дней,
даты по UTC. Поля записи: `symbol`, `dealAmount`, `fee`, `feeDeduction`, `rebateRate`,
`commissionAsset`, `commission`, `totalRebateUsdtAmount`, `settleStartTime`, `settleEndTime`,
`updateTime`.

Это меняет одно наше допущение: сверка идёт **по паре и монете**, а не одним запросом за
день по всем инструментам. Значит фоновая задача §4.4 ходит по списку пар ученика, а не
разом, и укладывается в вес запросов.

**OAuth Fast Connect.** WEEX предлагает подключение счёта без ручного ввода ключей -
трейдер связывает аккаунт «в несколько нажатий». Это прямо отвечает на вопрос, который
стоял в мультибиржевом ТЗ: ключи или вход биржей. На WEEX возможны оба пути, и вход
биржей для ученика проще и безопаснее ручного ключа. Условия и требования к брокеру для
Fast Connect - ещё один вопрос к BD.
...
- **Условные заявки** (стопы и цели) уходят с `clientAlgoId` (≤ 32 символов). WEEX
  подтвердила 12 сентября 2026: условные заявки засчитываются, метку брокера надо класть
  именно в `clientAlgoId`. **Сделано 13 сентября 2026**: клиент метит оба поля, у
  условных заявок своя метка с пределом 32 (`weex_algo_mark`).

  Одного этого было мало. Прежний ярлык защиты выглядел как `tp1_BTCUSDT-1789248000000` -
  двадцать пять знаков, и метка в тринадцать в предел уже не влезала: заявка ушла бы без
  неё. Поэтому ярлык стал коротким - `t1` плюс восемь знаков хеша сделки
  (`short_id` в [watcher.py](../../backend/trading/watcher.py)), это и есть тот самый
  «короткий биржевой идентификатор» из плана. Прежнее написание осталось в сравнении
  (`take_labels`, `stop_labels`): на бирже висят заявки, поставленные старым кодом, и
  защиту у них отнимать нельзя.
...
- **Админ, переключатель бирж (задача на будущее, запрос заказчика 11.09.2026):** в панели
  администратора выбирать биржу из подключённых (WEEX, BingX, OKX и следующие) и смотреть по
  ней партнёрские данные - рефералов, оборот, комиссию и кэшбэк. Сейчас партнёрка в админке
  только по WEEX (`backend/api/admin_affiliate.py`). Для переключателя нужны общий интерфейс
  партнёрского отчёта по биржам и параметр `exchange` в ручках админки; начисления кэшбэка
  уже хранят биржу в поле `exchange`.
...
Отдельно **WEEX** - отвечено 12 сентября 2026, см. §3:

- ~~Засчитываются ли условные заявки с `clientAlgoId`~~ - да, метку класть в `clientAlgoId`.
- ~~Складываются ли брокерский ребейт и партнёрская комиссия~~ - не складываются, нынешние
  рефералы остаются на партнёрских правилах.
- ~~Ставка и уровни~~ - обычно 50%, выше 65% отдельной заявкой; точное значение у BD.
