...
#
# Третьим доводом можно попросить сжатую ленту (`aggTrade`) и сравнить числа с
# обычной. Сжатая везёт все сделки одной заявки по одной цене одним сообщением,
# и это главный способ снять нагрузку: обычная на пяти монетах даёт 580
# сообщений в секунду, на полусотне тысячи. Проверять её нужно именно так, а не
# правкой сервера: в одной проверке она молчала вовсе - соединение поднялось, а
# сообщений не пришло ни одного, и на бою это оставило бы скринер без ленты.
DEPTH_RATE = "500ms"
TAPE_STREAM = "trade"
